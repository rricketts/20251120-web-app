...
export function applyFilter<T extends Record<string, any>>({
  ...
  if (filterName) {
    filteredData = filteredData.filter((item) =>
      item.backlink?.toLowerCase().includes(filterName.toLowerCase()) ||
      item.anchor_text?.toLowerCase().includes(filterName.toLowerCase()) ||
      item.anchor_url?.toLowerCase().includes(filterName.toLowerCase())
    );
  }
